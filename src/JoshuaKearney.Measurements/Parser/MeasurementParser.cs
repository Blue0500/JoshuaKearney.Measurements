...
//    public sealed class MeasurementParser<T> where T : IMeasurement<T> {
//        private static Regex NumericPattern { get; } = new Regex(@"^\d*\.?\d*$");

//        private Dictionary<string, MeasurementToken> dictionary;

//        private MeasurementSupplier<T> provider;

//        public MeasurementParser(MeasurementSupplier<T> p) {
//                provider = p;
...
//                    return new Union<T, MeasurementParseException>((T)final);
//                },
//                exception => exception
//            );
//        }
...
//        private static Dictionary<string, MeasurementToken> GetUnits<E>(MeasurementSupplier<E> provider) where E : Measurement<E> {
...
//            if (provider.GetType().GetTypeInfo().BaseType != null && provider.GetType().GetTypeInfo().BaseType.GetGenericTypeDefinition() == typeof(CompoundMeasurementSupplier<,,>)) {
